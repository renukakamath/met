...
@staff. route('/staffviewocc',methods=['post','get'])
def staffviewocc():
	data={}
	q="select * from occurrence inner join users USING(user_id)"
	r=select(q)
	data['message']=r
	if "submit" in request.form:
		m=request.form['msg']
		...
		q="update  occurrence set status='%s' where oc_id='%s'"%(m,mid)
		res=update(q)
		return redirect(url_for('staff.staffviewocc'))

	return render_template('staffviewocc.html',data=data)